      ...
      <tr>
        ...
        <td>
          ...
          <form action="{{route('admin.users.update',$user)}}" method="POST">
            ...
            @endforeach
            <button type="submit" class="float-left mr-1 btn-xs btn btn-success">Allow access</button>
          </form>
          @else
          ...
          <form action="{{route('admin.users.update',$user)}}" method="POST">
            ...
            <button type="submit" class="float-left mr-1 btn btn-xs btn-warning">Demote</button>
          </form>
          @else
          <form action="{{route('admin.users.update',$user)}}" method="POST">
            @csrf
            ...
            @endforeach
            <button type="submit" class="float-left mr-1 btn btn-xs btn-primary">Promote</button>
          </form>
          @endif
          @endif
          @can('edit-users')
          <a href="{{route('admin.users.edit',$user->id )}}"> <button type="button"
              class="float-left mr-1 btn btn-info">Edit</button></a>
          @endcan
          @can('delete-users')
